...
func TestFilterCredential(t *testing.T) {
	...
	criterionHolder := CriteriaHolder{Criterion: criterion}
	selectedAttributes := []string{"Apple", "Banana", "Cherry"}
	credentialB64Enc := base64.StdEncoding.EncodeToString([]byte(credentialJSON))
	filteredJSONBase64, err := FilterCredential(&criterionHolder, credentialB64Enc, selectedAttributes)
	assert.NoError(t, err)

	filteredJSON, err := base64.StdEncoding.DecodeString(filteredJSONBase64)
	assert.NoError(t, err)

	var filtered credential.UnsignedVerifiableCredential
	assert.NoError(t, json.Unmarshal(filteredJSON, &filtered))
